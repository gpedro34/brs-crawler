...
def.crawler.useUtilsCrawler = process.env.USE_UTILS_CRAWLER || def.crawler.useUtilsCrawler;
...
class Peers {
		  ...
		  switch(isIP){
				case 0:
					...
					const old = await this.db.execute(''
						+'SELECT ip FROM checks '
						+'WHERE peer_id = ? AND blocked = ?',
						[
							peer.id,
							BLOCK_REASONS.UNREACHABLE
						]
					);
					dns.resolve(add, (err, addrs) => {
						if(err){
							// handle it
							console.log(err);
						}
	...
	async updateIP(ip, reason, peerId){
		...
		if (res.affectedRows === 0) {
			...
			const hash = require('crypto').createHash('md5').update(id+','+ip).digest('hex');
			await this.db.execute(''
