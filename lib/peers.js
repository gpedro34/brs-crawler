...
class Peers {
	...
	async scan(userAgent, rescanInterval) {
	...
	async scanPeer(peer, userAgent) {
		...
		try {
			[resInfo,resPeers] = await Promise.all([
				utils.callBRS(peerUrl, userAgent, utils.BRS_REQUESTS.INFO),
				utils.callBRS(peerUrl, userAgent, utils.BRS_REQUESTS.PEERS),
			]);
		} catch (err) {
			if (err.cause) {
