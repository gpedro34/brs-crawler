...
class Peers {
	...
	async updatePeer(peer, peerInfo, peerDifficulty, rtt, peersCount) {
		this.db.execute(''
			+'UPDATE peers '
	...
	async scanPeer(peer, userAgent, timeout) {
		...
		let resInfo, resPeers, resDifficulty;
		try {
			[resInfo,resPeers,resDifficulty] = await Promise.all([
		...
		await Promise.all([
			this.updatePeer(peer, resInfo, resDifficulty, rtt, resPeers.peers.length),
